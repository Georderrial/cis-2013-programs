...
var passwordCheckOld = function () 
{
  ...
  if (stringPassword.length <8) //verify password less than 8 characters 
  {
    stringOutput= "There is a problem with password"; //error message
        
   }
   else
      {
   	for (var a=0; a < arrayBadPasswords.length; a++) //cycle each word of array
         {
            if (stringPassword === arrayBadPasswords[a]) //verify user input is equal to array
            {
                stringOutput = "This is a bad password"; //error message
            }
            else
            {
              stringOutput = "This is a great password"; //successful password
            }
         }

      }
      $("output").value = stringOutput; //sending message to screen
};
